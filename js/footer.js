(function footer(){
    var foot=`
    <div class="footer_white">
        <div class="w_1200">
            <a href="#">版权保护</a>
            <em>|</em>
            <a href="#">网站地图</a>
            <em>|</em>
            <a href="#">媒体链接</a>
        </div>
    </div>
    <!-- 最下面 -->
    <div class="footer_bottom">
        <div class="w_1200">
            <div class="top_link">
                <div class="right"  style="padding-right: 10px">
                        <img src="../恒力集团/wechat_90.png" alt="">
                        <img src="../恒力集团/wechat_90_2.png" alt="">
                </div>
                <div class="left">
                    <a href="#" class="footer_link">恒力报数字版链接</a>
                    <select class="select_box">
                        <option value="#">恒力下属企业链接</option>
                        <option value="#">恒力石化股份有限公司</option>
                        <option value="#">恒力石化（大连长兴岛）产业园</option>
                        <option value="#">江苏恒力化纤股份有限公司</option>
                        <option value="#">江苏德力化纤有限公司</option>
                        <option value="#">江苏恒科新材料有限公司</option>
                        <option value="#">营口康辉石化有限公司</option>
                    </select>
                    <div class="t_t">
                        <p>江苏省苏州市吴江区盛泽镇南麻恒力路1号 </p>
                        <p>电话：0512-63838999</p>
                        <p>传真：0512-63838877</p>
                    </div>
                </div>
            </div>
            <div class="bottom_text">
                <div class="text_left">
                    Copyright © 1994-2019 By 恒力集团有限公司版权所有 All Rights Reserved
                </div>
                <div class="text_right">
                    <a href="#">苏ICP备11074258号</a>
                    <a href="#">
                        <img src="../恒力集团/ghs.png" style="vertical-align: middle;margin:0 2; ">
                        苏公网安备 32050902100486号
                    </a>
                </div>
            </div>
        </div>
    </div>
    `
    document.getElementById("footer").innerHTML=foot;
})();